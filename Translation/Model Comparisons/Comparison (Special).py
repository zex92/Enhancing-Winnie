...
def Create_Pipeline(input_lang, output_lang):
    # Load Translation tokenizer and model based on language direction
    if input_lang == "English" and output_lang == "Luganda":
        model_name = "Helsinki-NLP/opus-mt-en-lg"
        weight_path = "E:/MODELs/Eng2Lug(Special).pth"
    elif input_lang == "Luganda" and output_lang == "English":
        model_name = "Helsinki-NLP/opus-mt-lg-en"
        weight_path = "E:/MODELs/Lug2Eng(Special).pth"
    else:
        raise ValueError("Unsupported language direction")

    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = load_translation(weight_path, model_name)

    translationModels = {input_lang: FullModel(model, tokenizer)}
...
def run():
    ...
    pipeline = Create_Pipeline(input_lang, output_lang)
